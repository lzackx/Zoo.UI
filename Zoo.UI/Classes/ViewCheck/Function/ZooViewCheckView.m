...
@interface ZooViewCheckView()

@property (nonatomic, strong) UIView *viewBound;//当前需要探测的view的边框
@property (nonatomic, strong) ZooVisualInfoWindow *infoWindow;//顶部被探测到的view的信息显示的UIwindow
...
@implementation ZooViewCheckView

-(instancetype)init{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(ZooScreenWidth/2-kViewCheckSize/2, ZooScreenHeight/2-kViewCheckSize/2, kViewCheckSize, kViewCheckSize);
        ...
        _viewBound.layer.borderColor = [UIColor zoo_colorWithHex:0xCC3A4B].CGColor;
        _viewBound.layer.zPosition = FLT_MAX;
        
        CGRect infoWindowFrame = CGRectZero;
        if (kInterfaceOrientationPortrait) {
            infoWindowFrame = CGRectMake(kZooSizeFrom750_Landscape(30), ZooScreenHeight - kZooSizeFrom750_Landscape(180) - kZooSizeFrom750_Landscape(30), ZooScreenWidth - 2*kZooSizeFrom750_Landscape(30), kZooSizeFrom750_Landscape(180));
        } else {
            infoWindowFrame = CGRectMake(kZooSizeFrom750_Landscape(30), ZooScreenHeight - kZooSizeFrom750_Landscape(180) - kZooSizeFrom750_Landscape(30), ZooScreenHeight - 2*kZooSizeFrom750_Landscape(30), kZooSizeFrom750_Landscape(180));
        }
...
-(NSMutableAttributedString *)viewInfo:(UIView *)view{
    if (view) {
        ...
        while(tempView != nil && tempView != self.zoo_viewController.view) {
        ...
        if (!ivarName) {
            ivarName = [self nameWithInstance:view inTarger:self.zoo_viewController.view];
        }
        
        if (!ivarName) {
            ivarName = [self nameWithInstance:view inTarger:view.zoo_viewController];
        }
        
        NSMutableString *showString = [[NSMutableString alloc] init];
        NSString *tempString = nil;
        if (ivarName) {
            tempString = [NSString stringWithFormat:@"%@:%@(%@)",ZooLocalizedString(@"控件名称"),NSStringFromClass([view class]),ivarName];
        }else{
            tempString = [NSString stringWithFormat:@"%@:%@",ZooLocalizedString(@"控件名称"),NSStringFromClass([view class])];
        }
        NSLog(@"tempString == %@",tempString);
        [showString appendString:tempString];
        
        tempString = [NSString stringWithFormat:ZooLocalizedString(@"\n控件位置：左%0.1lf  上%0.1lf  宽%0.1lf  高%0.1lf"),view.frame.origin.x,view.frame.origin.y,view.frame.size.width,view.frame.size.height];
        [showString appendString:tempString];
        ...
        style.lineSpacing = kZooSizeFrom750_Landscape(12);
        ...
        [attrString addAttributes:@{
                                    ...
                                    NSFontAttributeName : [UIFont systemFontOfSize: kZooSizeFrom750_Landscape(24)],
                                    NSForegroundColorAttributeName : [UIColor zoo_black_1]
                                    }
